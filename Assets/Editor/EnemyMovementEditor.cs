...
[CustomEditor(typeof(EnemyHiderScript))]
public class EnemyMovementEditor : Editor
{
    ...
    {
        EnemyHiderScript movement = (EnemyHiderScript)target;
        if (movement == null || movement.Player == null)
        {
            return;
        }

        int Hits = Physics.OverlapSphereNonAlloc(movement.Agent.transform.position, movement.LineOfSightChecker.checkCollider.radius, Colliders, movement.HidableLayers);
        if (Hits > 0)
        {
            int HitReduction = 0;
